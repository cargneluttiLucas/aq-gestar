...
export class NGZListComponent implements AfterViewInit {
  ...
  @ViewChild('box', { static: true }) boxList;
  ...
  ngAfterViewInit() {
    ...
    const rightElementRadio = this.boxList.nativeElement.querySelectorAll('app-radiobutton');
    if (rightElementRadio.length > 0) {
      this.isRadio = true;
    }

    const rightElementCheckbox = this.boxList.nativeElement.querySelectorAll('app-checkbox');
