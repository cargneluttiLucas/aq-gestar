...
export class SelectResultsComponent implements AfterViewInit, OnInit, OnChanges, IModalDialogOptions<any>, OnDestroy {
  childComponent?: any;
  placeOnTop?: boolean;
  isOverlay?: boolean;
  ...
  setModalMobile(): void {
    ...
    (this.documentService.nativeDocument.querySelector('app-select-results') as HTMLElement).style.alignItems = 'center';
    (this.documentService.nativeDocument.querySelector('app-select-results') as HTMLElement).style.border = '0';
    (this.documentService.nativeDocument.querySelector('app-select-results') as HTMLElement).style.display = 'flex';
    (this.documentService.nativeDocument.querySelector('app-select-results') as HTMLElement).style.height = 'calc(100vh + 10px)';
    (this.documentService.nativeDocument.querySelector('app-select-results') as HTMLElement).style.justifyContent = 'center';
    (this.documentService.nativeDocument.querySelector('app-select-results') as HTMLElement).style.left = '0%';
    (this.documentService.nativeDocument.querySelector('app-select-results') as HTMLElement).style.overflow = 'hidden';
    (this.documentService.nativeDocument.querySelector('app-select-results') as HTMLElement).style.position = 'fixed';
    (this.documentService.nativeDocument.querySelector('app-select-results') as HTMLElement).style.right = '0';
    (this.documentService.nativeDocument.querySelector('app-select-results') as HTMLElement).style.top = '-4%';
    (this.documentService.nativeDocument.querySelector('app-select-results') as HTMLElement).style.width = '100%';
    (this.documentService.nativeDocument.querySelector('app-select-results') as HTMLElement).style.zIndex = '850';
  }
