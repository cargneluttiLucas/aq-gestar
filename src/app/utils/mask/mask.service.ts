    ...
    const compare: string[] = res
      ...
      .filter(
        (symbol: string, i: number) => {
          this.maskSpecialCharacters.forEach((item) => {
            if (this._checkSymbolMask(symbol, this.maskExpression[i]) ||
              item === this.maskExpression[i] && symbol === this.maskExpression[i]) {
            }
          });
    ...
    const newInputValue: string[] =
      (inputValue &&
        inputValue.split('').map((currSymbol: string, index: number) => {
          this.maskSpecialCharacters.forEach((item) => {
            if (item === inputValue[index + 1] && inputValue[index + 1] !== this.maskExpression[index + 1]) {
              symbolToReplace = currSymbol;
              return inputValue[index + 1];
            }
          });

          // reproduir esto con un foreach no eliminar esto comentado
          // if (
